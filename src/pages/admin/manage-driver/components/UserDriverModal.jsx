...
// ==============================================================================
const UserDriverModal = ({ isOpen = false, isEdit = false, initialData = null, drivers = [], nextDriverId = "DSHB-DRV-01", isSubmitting = false, onClose, onSubmit }) => {
  const [formData, setFormData] = useState({
    id_driver: "DSHB-DRV-01",
    nama_lengkap: "",
    email: "",
    password: "",
    password_admin: "",
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEdit && initialData) {
        ...
        setFormData({
          id_driver: nextDriverId || "DSHB-DRV-01",
          nama_lengkap: "",
          email: "",
          password: "",
          password_admin: "",
  ...
  }, [isOpen, isEdit, initialData, nextDriverId]);
  ...
  const hasValidationError = isEdit
    ...
    : Boolean(duplicateAddName) || Boolean(duplicateAddEmail) || isInvalidEmailDomain;
  ...
  const handleSubmitForm = (e) => {
    ...
    if (duplicateAddName || duplicateAddEmail || duplicateEditName || duplicateEditEmail) {
    ...
    onSubmit({
      ...formData,
      id_driver: isEdit ? formData.id_driver : nextDriverId || formData.id_driver,
    });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-[fadeIn_0.15s]">
      <div className="bg-white rounded-2xl max-w-md w-full shadow-xl border border-slate-200 overflow-hidden text-left">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h4 className="text-base font-bold text-[#00206B] m-0">{isEdit ? "Edit Data Driver" : "Tambah Driver"}</h4>
            <p className="text-[11px] text-slate-400 mt-0.5">{isEdit ? "Perbarui nama atau reset kata sandi akun driver." : "Daftarkan akun pengemudi baru ke dalam sistem."}</p>
          </div>
          <button type="button" onClick={onClose} className="text-slate-400 hover:text-slate-600 p-1.5 rounded-lg hover:bg-slate-100 transition-colors cursor-pointer">
        ...
        <form onSubmit={handleSubmitForm} className="p-6 space-y-4" autoComplete="off">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full bg-white border text-sm font-semibold text-slate-900 rounded-xl px-4 py-2.5 outline-none focus:border-[#00206B] focus:ring-2 focus:ring-[#00206B]/10 transition-all ${
                (isEdit ? duplicateEditName : duplicateAddName) ? "border-rose-400 bg-rose-50/20 focus:border-rose-500" : "border-slate-200"
              }`}
              autoFocus
            />
          ...
          {isEdit ? (
            <div>
              <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">EMAIL AKUN DRIVER (TERKUNCI)</label>
            ...
            <div>
              ...
              <input
                ...
                className={`w-full bg-white border text-sm font-semibold text-[#00206B] rounded-xl px-4 py-2.5 outline-none focus:border-[#00206B] focus:ring-2 focus:ring-[#00206B]/10 transition-all ${
                  duplicateAddEmail || isInvalidEmailDomain ? "border-rose-400 bg-rose-50/20 focus:border-rose-500" : "border-slate-200"
                }`}
              />
          ...
          {/* 3. Password */}
          <div>
            <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">{isEdit ? "PASSWORD BARU (OPSIONAL)" : "PASSWORD LOGIN"}</label>
            <div className="relative">
              <input
                ...
                onChange={handleChange}
                placeholder={isEdit ? "Kosongkan jika tidak diganti" : "Contoh: Example27#"}
            ...
            <p className="text-[11px] text-slate-400 mt-1 font-normal">
              {isEdit ? "Kosongkan jika tidak diganti. Min. 8 karakter kombinasi huruf kapital, angka, dan simbol." : "Wajib min. 8 karakter, kombinasi huruf kapital, angka, dan simbol."}
            </p>
          </div>

          {/* 4. ID Driver (Otomatis & Terkunci) */}
          <div>
            <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">{isEdit ? "ID DRIVER (TERKUNCI)" : "ID DRIVER (OTOMATIS)"}</label>
            <div className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-slate-100 border border-slate-200 text-sm font-semibold text-slate-700 select-none">
              <span>{isEdit ? formData.id_driver : nextDriverId}</span>
              <div className="flex items-center gap-1.5 text-slate-400 text-xs font-normal">
                <span>{isEdit ? "Terkunci" : "Urut Otomatis"}</span>
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
          ...
          {/* 5. Security Notice Khusus Mode Edit */}
          {isEdit && (
            <div className="bg-amber-50/80 border border-amber-200/80 rounded-2xl p-3 flex items-start gap-2.5 text-amber-900 text-xs">
              ...
              <div className="leading-snug">
                ...
                <span className="text-[11px] text-amber-800">Masukkan password akun administrator Anda untuk mengonfirmasi perubahan data akun pengemudi ini.</span>
              </div>
            </div>
          )}

          {/* 6. Input Password Admin Konfirmasi */}
          {isEdit && (
            <div>
